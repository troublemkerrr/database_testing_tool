...
class AddTestCaseDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # 从数据库加载按钮名称并动态创建按钮
        button_names = self.load_button_names_from_db()

        # 为每个按钮创建并连接事件
        for name in button_names:
            button = QPushButton(name, self)
            button.clicked.connect(lambda _, btn_name=name: self.handle_button_click(btn_name))
    ...
    def init_db(self):
        """初始化 SQLite 数据库并创建表"""
        # 连接数据库（如果不存在会自动创建）
        self.conn = sqlite3.connect("preset_base_test.db")
    ...
    def load_button_names_from_db(self):
        """从数据库加载按钮名称"""
        self.cursor.execute('SELECT name FROM buttons')
    ...
    def handle_button_click(self, button_name):
        ...
        # 从数据库获取按钮对应的代码
        self.cursor.execute('SELECT code FROM buttons WHERE name = ?', (button_name,))
