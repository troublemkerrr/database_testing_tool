...
class TestCaseManager(QWidget):
    def __init__(self):
        ...
        self.test_case_cursor.execute('''CREATE TABLE IF NOT EXISTS test_cases (id INTEGER PRIMARY KEY, name TEXT, description TEXT, code TEXT, host TEXT, user TEXT, database TEXT)''')
    ...
    def add_test_case_to_db(self, name, description, code, host, user, database):
        # 将数据插入数据库
        self.test_case_cursor.execute('INSERT INTO test_cases (name, description, code, host, user, database) VALUES (?, ?, ?, ?, ?, ?)', (name, description, code, host, user, database))
        self.test_case_conn.commit()
